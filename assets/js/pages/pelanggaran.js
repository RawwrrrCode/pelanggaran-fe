...
async function load(page = 1) {
  currentPage = page;
  try {
    const res = await api.getPelanggaran(page);
    ...
    showAlert(alertEl, "danger", err?.data?.message || err.message || "Gagal load pelanggaran.");
  }
}
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    showAlert(alertEl, "success", "Data pelanggaran berhasil disimpan.");
    form.reset();
    await load(1);
  } catch (err) {
